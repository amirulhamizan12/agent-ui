...
import { CheckCircle, Clock, Loader2, AlertCircle, Play, Square } from 'lucide-react'
        ...
        <div className="mt-2 flex items-center space-x-3">
          ...
          {state.isRunning && state.taskId && (
            <button
              ...
              disabled={isStopping}
              className="text-red-400 hover:text-red-300 text-sm flex items-center space-x-1 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Square className="w-3 h-3" />
              <span>{isStopping ? 'Stopping...' : 'Stop Task'}</span>
            </button>
          )}
        </div>
      </div>
